...
const featuredProducts = [
  {
    ...
    rating: 4.9,
    stock: 'In stock',
    color: 'from-sky-500 to-bengal-cyan',
  },
  {
    ...
    rating: 4.7,
    stock: 'Few left',
    color: 'from-violet-500 to-bengal-blue-soft',
  },
  {
    name: 'Luma Daypack',
    price: '$79',
...
const featureList = [
  {
    icon: <Wallet className="h-5 w-5 text-bengal-cyan" />,
    title: 'Wallet Ready',
  ...
  {
    icon: <PackageOpen className="h-5 w-5 text-bengal-cyan" />,
    title: 'Inventory Control',
    description: 'Live stock updates with admin tools to keep every product listing accurate.',
  },
  {
    icon: <ShieldCheck className="h-5 w-5 text-bengal-cyan" />,
    title: 'Secure Orders',
  ...
  return (
    <main className="min-h-screen overflow-hidden">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-slate-50/95 backdrop-blur-lg">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-bengal-blue to-bengal-cyan text-white shadow-soft">
      ...
      <section id="home" className="relative overflow-hidden px-4 pb-16 pt-10 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-bengal-blue/10 px-4 py-2 text-sm text-bengal-blue">
              <span className="rounded-full bg-bengal-blue px-2 py-1 text-xs text-white">New</span>
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              <div className="rounded-3xl bg-white/90 p-6 shadow-soft border border-slate-200">
                <span className="text-3xl font-semibold text-bengal-blue">24K+</span>
                <p className="mt-3 text-sm text-slate-600">Active buyers every month</p>
              </div>
              <div className="rounded-3xl bg-white/90 p-6 shadow-soft border border-slate-200">
                <span className="text-3xl font-semibold text-bengal-blue">98%</span>
                <p className="mt-3 text-sm text-slate-600">Fast delivery satisfaction</p>
              </div>
              <div className="rounded-3xl bg-white/90 p-6 shadow-soft border border-slate-200">
                <span className="text-3xl font-semibold text-bengal-blue">1.8K</span>
                <p className="mt-3 text-sm text-slate-600">Verified reviews and ratings</p>
              </div>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} className="relative overflow-hidden rounded-[2.5rem] bg-bengal-blue text-white shadow-soft">
      ...
      <section id="products" className="px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.25em] text-bengal-cyan">Featured collection</p>
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => (
                <button key={category.value} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 transition hover:border-bengal-blue hover:text-bengal-blue">
                  {category.label}
                </button>
              ))}
            </div>
            ...
            {featuredProducts.map((product) => (
              <motion.div key={product.name} whileHover={{ y: -8 }} className="section-card overflow-hidden p-6">
                <div className="flex items-center justify-between">
                  ...
                  <button className="rounded-full bg-slate-100 p-2 text-slate-700 transition hover:bg-bengal-blue hover:text-white">
                ...
                <div className="mt-6 flex items-center justify-between gap-3">
                  <button className="inline-flex items-center gap-2 rounded-full bg-bengal-blue px-5 py-3 text-sm font-semibold text-white transition hover:bg-bengal-blue-soft">
                    <ShoppingBag className="h-4 w-4" />
                    Add to cart
                  </button>
                  <button className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-slate-200 text-slate-700 transition hover:border-bengal-blue hover:text-bengal-blue">
      ...
      <section id="categories" className="bg-slate-100 px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 text-center">
            <p className="text-sm uppercase tracking-[0.3em] text-bengal-cyan">Browse by category</p>
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
            {['New Arrivals', 'Wallet Deals', 'Fast Delivery', 'Verified Reviews'].map((item) => (
              <div key={item} className="section-card p-6">
                <div className="flex h-12 w-12 items-center justify-center rounded-3xl bg-bengal-blue text-white shadow-soft">
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_0.7fr] lg:items-center">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-bengal-cyan">Smart experience</p>
              ...
              <div className="mt-10 grid gap-6 sm:grid-cols-2">
                {featureList.map((feature) => (
                  <div key={feature.title} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-bengal-blue/5 text-bengal-cyan">{feature.icon}</div>
            ...
            <div className="rounded-[2rem] bg-gradient-to-br from-bengal-blue to-bengal-blue-soft p-8 text-white shadow-soft">
      ...
      <section className="bg-slate-100 px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 text-center">
            <p className="text-sm uppercase tracking-[0.3em] text-bengal-cyan">Customer feedback</p>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {reviews.map((review) => (
              <div key={review.name} className="section-card p-8">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-3xl bg-bengal-blue text-white">
      ...
      <section id="about" className="px-4 pb-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-[2rem] bg-white p-10 shadow-soft sm:p-14">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_0.7fr] lg:items-center">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-bengal-cyan">About the DApp</p>
            ...
            <div className="grid gap-4 rounded-3xl bg-slate-50 p-8">
              <div className="flex items-start gap-4">
                <div className="rounded-3xl bg-bengal-blue/5 p-3 text-bengal-blue"><MapPin className="h-5 w-5" /></div>
                <div>
              ...
              <div className="flex items-start gap-4">
                <div className="rounded-3xl bg-bengal-blue/5 p-3 text-bengal-blue"><Truck className="h-5 w-5" /></div>
                <div>
              ...
              <div className="flex items-start gap-4">
                <div className="rounded-3xl bg-bengal-blue/5 p-3 text-bengal-blue"><ShieldCheck className="h-5 w-5" /></div>
                <div>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:px-8 lg:grid-cols-[1.2fr_0.8fr_0.8fr]">
          ...
          <div>
            ...
            <form className="mt-6 flex flex-col gap-3 sm:flex-row">
              <input type="email" placeholder="Email address" className="min-w-0 flex-1 rounded-full border border-white/20 bg-white/10 px-5 py-3 text-sm text-white outline-none placeholder:text-slate-200 focus:border-bengal-cyan focus:bg-white/20" />
              <button className="rounded-full bg-bengal-cyan px-6 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300">Subscribe</button>
            </form>
          </div>
        </div>
